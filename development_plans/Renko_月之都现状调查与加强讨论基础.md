...
- 状态：**首批游戏实现已按用户要求全部撤回；保留调查和设计资料，按用户后续指挥逐项实施。初次刷兵待编制与编制AI确定后统一处理；“无尘战场规格”列为待重做，尚未修改。详见第19节。**
...
详细需求分类、参考来源及最新状态见[分段实施计划](Renko_LUN轻量AI加强分段实施计划.md)。批次编号仅用于分类，不再代表批准的完成顺序。

- 用户要求重新把关；初始刷兵依赖最终编制及编制AI，后续按用户逐项指挥。
- 已撤回本轮所有游戏实现：初次owner恢复ROOT；迁都、新补给精神、亚洲/中东州补给及新增反制/清理均移除。国策文件恢复本次实施前版本。
- “无尘战场规格”在新体系中的功能可能失去原有价值，登记待重新设计，未修改原精神和原“翻转主义”奖励。不得把助手新增的整套补给网拆除当作原作者既定要求。
- 地区补给敌我共享可接受、科技期限按世界日历年份的用户要求继续保留。
- 日本与特殊属国刷兵的owner写法已查，详见实施计划第12.3节；恢复ROOT不代表已经验证当前嵌套上下文的实际归属。
